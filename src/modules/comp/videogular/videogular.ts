...
export class Videogular implements OnInit, OnDestroy, AfterViewInit {
  ...
  selectQuality(quality: any): void {
    ...
    Object.keys(this.controlDebounceTimers).forEach(key => {
      if (this.controlDebounceTimers[key]) {
        clearTimeout(this.controlDebounceTimers[key]);
        this.controlDebounceTimers[key] = null;
      }
    });
  ...
  /**
   * 测试切换清晰度和重试逻辑清除功能
   * 用于验证修复的有效性，可在开发环境下使用
   */
  testQualitySwitchAndRetryClear(): void {
    console.log('开始测试切换清晰度和重试逻辑清除...');
    ...
    // 创建一个模拟的缓冲超时定时器
    this.bufferingTimeout = setTimeout(() => {
      console.log('不应该看到这个消息，如果修复生效');
    }, 1000);
    
    console.log('测试前状态:', { retryCount: this.retryCount, loadError: this.loadError, isBuffering: this.isBuffering, hasBufferingTimeout: !!this.bufferingTimeout });
    
    // 调用修改后的selectQuality方法
    const testQuality = this.availableQualities[0];
    this.selectQuality(testQuality);
    
    console.log('测试后状态:', { retryCount: this.retryCount, loadError: this.loadError, isBuffering: this.isBuffering, hasBufferingTimeout: !!this.bufferingTimeout });
  }
